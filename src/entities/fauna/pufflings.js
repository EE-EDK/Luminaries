...
export function makePuff(x, z) {
  ...
  for (let i = -1; i <= 1; i += 2) {
    ...
    innerEar.position.set(i * 0.13, 0.84, 0.01); innerEar.rotation.z = i * 0.3; shell.add(innerEar);
  ...
  for (let i = -1; i <= 1; i += 2) {
    ...
    chk.position.set(i * 0.15, 0.61, 0.15); shell.add(chk);
  }
  ...
  for (let i = -1; i <= 1; i += 2) {
    for (let j = 0; j < 2; j++) {
      ...
      wh.rotation.z = i * 0.7 + j * i * 0.2; wh.rotation.x = -0.1; shell.add(wh);
    }
  }
  ...
  for (let i = -1; i <= 1; i += 2) {
    ...
    brow.position.set(i * 0.09, 0.72, 0.17); brow.rotation.z = i * 0.3; shell.add(brow);
  }
  ...
  mouth.position.set(0, 0.59, 0.22); mouth.rotation.z = Math.PI / 2; shell.add(mouth);
  ...
  if (sr() < 0.4) {
    ...
    bud.position.set(0, 0.89, 0); shell.add(bud);
  }

  // --- Orbiting sparkle motes (stay on group, not shell — shouldn't squash) ---
  ...
  // Ground glow disc — stays on group (shouldn't squash)
  ...
  return {
    group: g, shell, body, head, ears, eyes, tail, sparkles, crownMat, bodyMat, glowMat,
