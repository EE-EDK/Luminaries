...
export function makeDandelion(x, z) {
  ...
  const rosetteMat = new MeshStandardMaterial({
    color: 0x2a6028, roughness: 0.7, side: DoubleSide
  });
  const rosN = 3 + Math.floor(sr() * 2);
  for (let ri = 0; ri < rosN; ri++) {
    const ra = (ri / rosN) * 6.28 + sr() * 0.3;
    const rLeaf = new Mesh(new PlaneGeometry(0.08, 0.035), rosetteMat);
    rLeaf.position.set(Math.cos(ra) * 0.05, 0.02, Math.sin(ra) * 0.05);
    rLeaf.rotation.x = -1.4; rLeaf.rotation.y = -ra; g.add(rLeaf);
  }
  // Stem
  const stemMat = new MeshStandardMaterial({ color: C.dandStem, roughness: 0.8 });
  const stem = new Mesh(new CylinderGeometry(0.008, 0.012, h, 4), stemMat);
  stem.position.y = h / 2; g.add(stem);
  // Stem fuzz (tiny hairs along stem)
  const fuzzMat = new MeshBasicMaterial({ color: 0xddddcc, transparent: true, opacity: 0.3 });
  for (let fi = 0; fi < 4; fi++) {
    const fy = h * 0.2 + fi * h * 0.18;
    const fuzz = new Mesh(new CylinderGeometry(0.002, 0.001, 0.02, 3), fuzzMat);
  ...
  const leaf = new Mesh(new PlaneGeometry(0.07, 0.03), leafMat);
  ...
  const head = new Mesh(new SphereGeometry(0.07, 6, 5), headMat);
  ...
  const dew = new Mesh(new SphereGeometry(0.012, 4, 3), dewMat);
  dew.position.set(0.04, h + 0.07, 0.03); g.add(dew);
  // Individual seed tufts (8 tiny cones radiating outward, up from 6)
  const tuftMat = new MeshBasicMaterial({ color: C.dandSeed, transparent: true, opacity: 0.7 });
  for (let i = 0; i < 8; i++) {
    const ta = (i / 8) * 6.28 + sr() * 0.2;
    const tuft = new Mesh(new ConeGeometry(0.015, 0.05, 3), tuftMat);
    ...
    const filMat = new MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.3 });
    const fil = new Mesh(new CylinderGeometry(0.001, 0.001, 0.03, 3), filMat);
    ...
    const papMat = new MeshBasicMaterial({ color: 0xeeeedd, transparent: true, opacity: 0.15 });
    for (let pfi = 0; pfi < 2; pfi++) {
      const pfA = ta + (pfi - 0.5) * 0.5;
      const pap = new Mesh(new CylinderGeometry(0.0008, 0.0008, 0.025, 3), papMat);
  ...
  const achMat = new MeshStandardMaterial({ color: 0x3a2a10, roughness: 0.9 });
  for (let ai = 0; ai < 4; ai++) {
    const aa = (ai / 4) * 6.28 + sr() * 0.5;
    const ach = new Mesh(new SphereGeometry(0.005, 3, 3), achMat);
  ...
  const grooveMat = new MeshBasicMaterial({ color: 0x1a3010, transparent: true, opacity: 0.15 });
  for (let gi = 0; gi < 3; gi++) {
    const ga = (gi / 3) * 6.28;
    const groove = new Mesh(new CylinderGeometry(0.001, 0.001, h * 0.7, 3), grooveMat);
    groove.position.set(Math.cos(ga) * 0.01, h * 0.35, Math.sin(ga) * 0.01);
    g.add(groove);
  }

  // Micro-trichomes (tiny hairs on stem surface)
  const trichMat = new MeshBasicMaterial({ color: 0xccddbb, transparent: true, opacity: 0.2 });
  for (let tri = 0; tri < 5; tri++) {
    const tY = h * 0.1 + sr() * h * 0.7;
    const tA = sr() * 6.28;
    const trich = new Mesh(new CylinderGeometry(0.0008, 0.0008, 0.012, 3), trichMat);
