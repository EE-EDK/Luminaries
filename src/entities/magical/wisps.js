...
export function makeWisp(x, y, z) {
  const g = new Group();
  // Core bright sphere
  const coreMat = new MeshBasicMaterial({ color: C.wispCore });
  const core = new Mesh(new SphereGeometry(0.08, 6, 4), coreMat);
  g.add(core);
  ...
  for (let pi = 0; pi < 3; pi++) {
    ...
    const plasma = new Mesh(new CylinderGeometry(0.003, 0.001, pLen, 3), plasmaMat);
